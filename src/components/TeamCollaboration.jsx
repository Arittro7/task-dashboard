...
const TeamCollaboration = ({ users }) => {
  const team = users.slice(0, 4).map((user, idx) => ({
    ...
    workingOnDetails: [
      ...
      "Development and Filter Functionality",
      "Responsive Language",
    ][idx],
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md h-full">
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="text-primary font-medium border border-gray-300 rounded-full px-3 py-1">
          + Add Member
        </button>
      </div>
      {team.map((member, idx) => (
        <div key={idx} className="flex items-center mb-4">
          ...
          <div className="flex-1 text-left ml-2.5">
            <p className="font-medium text-gray-800 text-xl font-semibold">
              {member.name}
            </p>
            <p className="text-sm text-gray-500">
              ...
              <span className="text-black font-semibold">
                {" "}
                {member.workingOnDetails}
              </span>
            </p>
          </div>
          <span
            className={`px-3 py-1 rounded text-sm bg-${member.statusColor} text-${member.textColor}`}
          >
            {member.status}
          </span>
        </div>
